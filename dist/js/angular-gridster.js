		...
		function($scope, $rootScope, gridsterConfig) {
			...
			this.resolveOptions = function() {
				...
				if ($scope.options.isLoaded === true) {
					$rootScope.$broadcast('angular-gridster.grid_changed');
					if (modeChanged === true) {
			 ...
			 */
			this.setElementHeight = function(el, sizeX, sizeY) {
				var height;
				if (!$scope.options.isLoaded) {
					// need to make sure grid isLoaded so css height transition isn't clobbered
					return 0;
				}
				height = sizeY * $scope.options.curRowHeight - this.getOption('margins')[0];
				if (el === null) {
					el = previewElement;
				}
				el.style.height = height + 'px';
				if (el !== previewElement && this.getOption('updates') && this.getOption('updates').height) {
					this.getOption('updates').height(el, height);
				}
				return height;
			};
			/**
			...
			this.setElementWidth = function(el, sizeX) {
				...
				width = sizeX * $scope.options.curColWidth - this.getOption('margins')[1] + 'px';
			...
			this.setElement = function(el, item, ignoreHeight) {
				...
				if (ignoreHeight !== true) {
					this.setElementHeight(el, this.getSizeX(item), this.getSizeY(item));
				}
			};
			/**
		...
		function($timeout) {
			return {
				...
				link: function(scope, $element, attrs, gridster) {
					...
					// watch item model for changes and update in grid
					scope.$watch('item', function(newVal, oldVal) {
						if (newVal !== oldVal && newVal._moving !== true) {
							scope.item = gridster.fixItem(newVal);
						...
						var resizeMove = function(e, width, height) {
							...
							if (widthChanged || heightChanged) {
								...
								if (gridster.getOption('resizablePreviewEnabled') === true) {
									...
									gridster.setElementHeight(null, sizeX, sizeY);
								}
							}
						...
						var resizeEnd = function(e) {
							...
							scope.$apply(function() {
								...
								if (gridster.hasItemWidthChanged(scope.item, sizeXStart) === false && gridster.hasItemHeightChanged(scope.item, sizeYStart) === false) {
									...
									gridster.setElementHeight(element, sizeX, sizeY);
								}
								gridster.floatItemsUp();
								gridster.endMove();
					...
					scope.$on('angular-gridster.loaded', function() {
						gridster.setElementHeight(element, gridster.getSizeX(scope.item), gridster.getSizeY(scope.item));
						setInteractions();
					});
					scope.$on('$destroy', function() {
